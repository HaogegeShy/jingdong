...
import Toast, { useToastShowEffect } from '@/components/Toast.vue'
...
export default {
  name: 'Login',
  components: { Toast },
  setup () { // setup职责就是告诉你，代码执行的一个流程
    const { show, toastMessage, showToast } = useToastShowEffect()
    const { handleLogin, username, password } = useLoginEffect(showToast)
    const { handleRegister } = useRegisterEffect()
    return { show, toastMessage, handleLogin, handleRegister, username, password }
  }
}
</script>
